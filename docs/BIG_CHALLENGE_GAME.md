...
> **版本**：v1.2 | **建立日期**：2026-03-25 | **最後更新**：2026-03-25
...
名單依**小隊分組**顯示，每位成員標示三種狀態之一：

| 狀態 | 說明 | 顯示色 |
|------|------|--------|
| 已報名 | 已報名本活動（若已核銷則顯示「✅ 已出席」） | 紫色 / 綠色 |
| 未報名 | 尚未報名任何巔峰試煉 | 灰色 |
| 跨出報名 | 已報名**別大隊**的巔峰試煉，顯示「跨出→XXX大隊」 | 琥珀色 |

**跨出報名邏輯**：若成員未報名本活動，但在 `PeakTrialRegistrations` 中存在其他 `trial_id`（對應不同 `battalion_name` 的活動），即標記為「跨出」，並顯示目標大隊名稱。同一人若跨出多個大隊，只顯示第一筆。

### 5.4 跨入名單

點擊「跨入名單」按鈕（琥珀色），查詢**非本大隊**成員報名本活動的記錄。

每筆顯示：姓名、原始大隊、原始小隊、出席狀態。

> 兩個名單視圖互斥，再次點擊同一按鈕收起；切換按鈕直接替換顯示。資料每次點擊時從 Supabase 即時拉取（`getBattalionTrialStatus`）。
...
| `getBattalionTrialStatus(battalionName, trialId)` | 查詢本隊成員報名狀態（已報名 / 未報名 / 跨出）+ 跨入名單 |
...
| CommandantTab 巔峰試煉管理區塊 | `components/Tabs/CommandantTab.tsx` | 大隊長活動 CRUD + 報名名單 + 本隊名單 + 跨入名單 + 掃碼 |
...
7. **跨出報名判斷**：同一成員在不同 `battalion_name` 的活動中存在報名記錄即視為「跨出」；同時跨出多個大隊時只顯示第一筆
8. **跨入名單過濾**：以 `PeakTrialRegistrations.battalion_name ≠ 本大隊名稱` 判定，依賴報名時正確帶入學員的 `BigTeamLeagelName`
